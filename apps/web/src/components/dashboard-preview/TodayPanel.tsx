...
function SeatMeter({ session, state }: { session: Session; state: 'past' | 'next' | 'later' }) {
  ...
  return (
    <div className="w-[74px] shrink-0 text-right">
      ...
      {showAttendance ? (
        <p className="mt-0.5 text-[0.6875rem] leading-none text-muted-foreground">{t('seatsAttended')}</p>
      ) : cap != null && cap > 0 ? (
        ...
        <p className="mt-0.5 text-[0.6875rem] leading-none text-muted-foreground">{t('seatsBooked')}</p>
      )}
      {trials > 0 && (
        <p className="mt-1 text-[0.6875rem] leading-none text-emerald-600">
