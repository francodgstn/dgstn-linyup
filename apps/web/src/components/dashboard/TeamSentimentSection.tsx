 ...
 *
 * One reading of the whole team, made from the contact summaries the studio has
 * already generated: a mood, the overall picture, what is working, what to watch
 * and where to focus. Written by `generateTeamSentiment` into
 * `teams/{teamId}/ai_reports/team_sentiment`, read here with one listener — the
 * same document carries today's run count, so "3 of 5 left today" is never a
 * second read.
 *
 * ── WHERE IT SITS ────────────────────────────────────────────────────────────
 ...
 *
 * ── IT SAYS HOW MUCH IT READ ─────────────────────────────────────────────────
 * Contact summaries are generated by hand, so a reading can rest on eight people
 * in a roster of two hundred. The footer always states the count and how old the
 * oldest summary was — a mood without its sample size is a claim nobody can
 * weigh.
 */
...
export function TeamSentimentSection({ teamId }: { teamId: string | null }) {
  ...
  async function generate() {
    if (!teamId || busy || runsLeft <= 0) return
    setBusy(true)
    try {
      await httpsCallable(functions, 'generateTeamSentiment')({ teamId })
      toast.success(t('sentimentGenerated'))
    } catch (err) {
      console.error('[team-sentiment] generate failed:', err)
      const reason = (err as { details?: { reason?: string } })?.details?.reason
      toast.error(
        reason === 'not_enough_summaries'
          ? t('sentimentNotEnough', { needed: TEAM_SENTIMENT_MIN_SUMMARIES })
          : reason === 'daily_limit'
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-t pt-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            size="sm"
            onClick={generate}
            disabled={busy || runsLeft <= 0 || !teamId}
            className="h-7 gap-1.5 text-xs"
          >
            <RefreshCw className={`h-3.5 w-3.5 ${busy ? 'animate-spin' : ''}`} />
            {report ? t('sentimentRefresh') : t('sentimentGenerate')}
          </Button>
        </div>
      </div>
