    ...
    <div ref={wrapRef} onMouseEnter={show} onMouseLeave={hide} className="relative">
      ...
      {open &&
        ...
        createPortal(
          ...
          >
            {label && (
              <p className="px-2 pb-1 pt-0.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground/60">
                {label}
              </p>
            )}
  ...
  return (
    <div className="mt-3">
      {collapsed ? (
        ...
        <p className="text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider px-2 pb-1">
          {label}
        </p>
      )}
    ...
    <div className={collapsed ? 'mt-3 pt-3' : 'relative mt-3 py-1'}>
      ...
      {!collapsed && (
        <div className="flex items-center pb-1">
          <p className="flex-1 px-2 text-xs font-medium text-muted-foreground/50">
            {t('navGroupShortcuts')}
          </p>
  ...
  const renderEntry = (entry: SearchEntry) => {
    ...
    const row = (
      ...
      >
        ...
        {entry.badge && (
          <span className="shrink-0 rounded border border-amber-500/40 bg-amber-500/10 px-1.5 py-px text-xs font-medium uppercase tracking-wide text-amber-700 dark:text-amber-400">
            {entry.badge}
          </span>
        )}
        {entry.sublabel && (
      ...
      {!showResults && (
        <>
          <div className="mt-1.5 flex items-center gap-2 rounded-md px-2 py-3 text-sm text-muted-foreground">
            ...
            <kbd className="ml-auto shrink-0 rounded border px-1.5 py-0.5 font-sans text-xs text-muted-foreground/70">
              {modKeyLabel()}K
            </kbd>
          </div>
          ...
          {recentContactRows.length > 0 && (
            ...
            >
              <p className="px-2 pb-1 pt-0.5 text-xs font-medium text-muted-foreground/50">
        ...
        >
          {flat.length === 0 ? (
            ...
            <>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1 border-t px-2 pb-0.5 pt-1.5 text-xs text-muted-foreground/70">
                <span>
