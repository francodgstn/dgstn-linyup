...
// The plugin marketplace — a FULL PAGE, at the prefix its per-plugin editors
// already use (/plugins/website, /plugins/finance, /plugins/kiosk, …). It lived
// at /settings/plugins from 2026-08 until 2026-09-20, inside the settings shell,
...
//
// NO NEW SIDEBAR ROW CAME WITH THE MOVE. Every way in is one that already
// existed: the "Explore plugins" icon in the sidebar's utility row, the Settings
// rail's Plugins row (the one row there that deliberately leaves the shell —
// lib/settings-nav.ts says why), the dashboard's Discover panel, the muted nav
// suggestions, `PluginNotInstalled`, the setup checklist, and the "Set up"
// buttons on /public-page. All of them deep-link with `?plugin=<id>`.
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          <div className="flex items-center gap-2">
            ...
            {manifest.locked && !isInstalled && (
              <span className="inline-flex items-center gap-1 rounded-full bg-muted px-1.5 py-0.5 text-xs font-medium text-muted-foreground shrink-0">
...
export default function PluginsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Search + category filter — ONE ROW once there is room for it. They were
          stacked when this page was 3xl-wide inside the settings pane; at full
          width a search box stretched across the screen reads as the page's main
          event rather than as a filter on the grid below. */}
