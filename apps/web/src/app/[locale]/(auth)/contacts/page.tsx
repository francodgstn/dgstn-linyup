  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent align="end" side="bottom" className="w-52 p-1.5">
        <p className="text-[0.625rem] font-semibold text-muted-foreground uppercase tracking-wider px-2 pt-1 pb-0.5">{t('savedPresetsHeading')}</p>
        {presets.map((q) => {
        ...
        {saved.length > 0 && (
          <>
            ...
            <p className="text-[0.625rem] font-semibold text-muted-foreground uppercase tracking-wider px-2 pb-0.5">{t('savedYoursHeading')}</p>
            {saved.map((q) => (
              <div key={q.id} className="flex items-center gap-1 rounded hover:bg-accent group px-1">
                <button type="button" onClick={() => apply(q)} className="flex-1 px-1 py-1.5 text-sm text-left truncate">{q.name}</button>
...
function ConsentFilterContent({ documents, value, onChange }: {
  ...
  return (
    <div className="min-w-[260px] p-1 space-y-1.5">
      <p className="text-[0.625rem] font-semibold text-muted-foreground uppercase tracking-wider px-1 py-0.5">
      ...
      {selected && !selected.requiredBeforeBooking && (
        <p className="px-2 pt-1.5 text-[0.6875rem] leading-snug text-muted-foreground">
  ...
  const DIMENSIONS: FilterDimension[] = [
    {
      ...
      render: (f, set) => (
        <div className="p-1 space-y-0.5">
          <p className="text-[0.625rem] font-semibold text-muted-foreground uppercase tracking-wider px-1 py-0.5">{t('filterStage')}</p>
          ...
          <div className="border-t my-1.5 mx-1" />
          <p className="text-[0.625rem] font-semibold text-muted-foreground uppercase tracking-wider px-1 py-0.5">{t('filterSource')}</p>
    ...
    },
    {
      ...
      render: (f, set) => (
        <div className="p-1 space-y-0.5">
          <p className="text-[0.625rem] font-semibold text-muted-foreground uppercase tracking-wider px-1 py-0.5">{t('filterLastActive')}</p>
          ...
          <div className="border-t my-1.5 mx-1" />
          <p className="text-[0.625rem] font-semibold text-muted-foreground uppercase tracking-wider px-1 py-0.5">{t('filterSessions')}</p>
  ...
  return (
    <div className="flex items-center hover:bg-muted/50 transition-colors">
        ...
        <div className="flex-1 min-w-0 space-y-0.5">
          ...
          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
            ...
            {contact.pending_signup && (
              <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[0.625rem] font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
                {t('pendingSignup')}
              </span>
            )}
      ...
      {/* WHY this row is near the top, shown only while the list is ordered by
          ...
          nobody trusts. */}
      {attentionReason && (
        <span className="shrink-0 px-2 text-[0.6875rem] font-medium text-amber-600 dark:text-amber-400">
