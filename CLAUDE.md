...
### Event programs — what makes an event more than a session

An event **has a program**: a multi-day, multi-track agenda. `Event.program`
...
- **Times are WALL-CLOCK at the venue** (`'HH:MM'` + the day's `'YYYY-MM-DD'`),
  never `Timestamp`s. A program is a printed schedule — "09:00 breakfast" is
...
- **Events are private by default.** `Event.publicVisibility` gates
  `syncEventPublicProfile`, which embeds the whole program into one mirror doc
...
`dayIndex`, so an org authors a standard camp program once and every member
...
- **A course OWNS a `session_series`** whose sessions are its lessons, which is
  what gets it a roster, attendance, reminders and teardown for free. The series
