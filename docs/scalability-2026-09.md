...
CHF default** (2026-09-18) — the alert path was already fixed, and the amount was
weighed and left, with the forecast rule and the Providers page carrying the
detection instead. **App Check is deferred** (2026-09-12). See the row above,
`infra/README.md` → "Picking `budget_amount`", and the runbook's "Why it is
still off". None of it was architectural — the data model is sound, and nothing
here required reshaping collections or the tenant boundary.
