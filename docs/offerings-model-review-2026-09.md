...
| **Plan ↔ offering edge** | access facet (`accessRule.subscriptionTypeIds`) + rate facet (`memberBenefit` / `durationBenefits` / `Course.benefit`) | **only on the offering**; one writer `activityPlanEdgeUpdate` | `gatedPlanIds`, `ratedPlanIds`, catalog |
...
| R4 | Course `benefit{included}` vs `accessRule.subscriptionTypeIds` | Shim absorbed on write; `firestore.rules` still honors the legacy spelling. |
| R5 | `introOffer` vs `introOffers[]` | Shim, normalized by `introOffersOf`. |
