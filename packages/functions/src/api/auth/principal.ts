...
// MCP, API key or (Phase 2) OAuth token, is answered by `resolveApiPrincipal` and
// by nothing else. The read layer then asks `principalMay` / `principalSees*`.
//
...
//      refuses.
//   3. NARROWER THAN THE RULES, NEVER WIDER. Scopes intersect capabilities; the
...
export interface ApiPrincipal {
  ...
  dataScope: DataScope
  /** The parent key's `last_used_at`, for the throttled touch. */
  lastUsedAtMs: number | null
}

/**
 * Which credential kinds may be presented as a bearer on each surface. OAuth
 * access tokens are bound to the MCP resource (RFC 8707), so they are refused
 * on REST; codes and refresh tokens are never bearers at all.
 */
...
/** Pure: the whole decision, given what was read. */
...
function parentRef(credential: ApiCredential): FirebaseFirestore.DocumentReference | null {
  const team = admin.firestore().collection(TEAMS_COLLECTION).doc(credential.teamId)
  if (credential.kind === 'api_key') return team.collection(API_KEYS_SUBCOLLECTION).doc(credential.parent_id)
  // OAuth grants arrive in Phase 2; until then an OAuth credential has no parent
  // to find, and so refuses as revoked.
  return null
}
...
export async function resolveApiPrincipal(
  ...
  // API keys are recognisable offline; OAuth access tokens (Phase 2) are not
  // lyp_-prefixed and skip this check.
  ...
  if (!credential) return decideApiPrincipal({ credential: null, parent: null, member: null }, surface, nowMs)

  const parent = parentRef(credential)
  const memberRef = db
    .collection(TEAMS_COLLECTION)
    .doc(credential.teamId)
    .collection(TEAM_MEMBERS_SUBCOLLECTION)
    .doc(credential.uid)
  const [parentSnap, memberSnap] = parent ? await db.getAll(parent, memberRef) : [null, await memberRef.get()]
