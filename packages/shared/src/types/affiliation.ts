...
export interface AffiliationSummary {
  ...
  /**
   * One entry per (org, status) the contact's org-issued affiliations sit in,
   * as `orgAffiliationStatusKey(org_id, status_id)` — `'hmd:under_review'`.
   *
   * IT EXISTS SO A STATUS BREAKDOWN CAN EXCLUDE THE PEOPLE WHO LEFT. Counting
   * the affiliation documents themselves is the obvious way to build one and it
   * cannot be filtered: `archived_at` lives on the PARENT contact and a
   * collection-group query has no way to reach it, so an ex-member's licence
   * stayed in the federation's queue for ever. The org dashboard's breakdown
   * read 34 records on a page whose headcount was 31 (Franco, 2026-09-08).
   * Asking the contact instead puts the status filter on the same document as
   * `archived_at`, so it composes with `liveContactConstraints()` like every
   * other count on that page.
   *
   * A COMPOSITE KEY BECAUSE FIRESTORE ALLOWS ONE `array-contains` PER QUERY.
   * Two arrays — orgs and statuses — would answer "some org of yours, and some
   * status of yours", which for a contact affiliated to two federations is a
   * different and wrong question.
   *
   * The set is DISTINCT, so a contact holding two affiliations from one org in
   * the same status is counted once: this counts PEOPLE, and the strip says so.
   * Holding two in DIFFERENT statuses does put them in both buckets, which is
   * why the strip never states these as a share of the headcount.
   *
   * OPTIONAL, with the same hazard as `active_org_ids` above: a summary written
   * before the field existed lacks it, `array-contains` never matches a missing
   * field, and such a contact drops OUT of the breakdown rather than being
   * counted wrongly. Visibly low beats invisibly high, but it is still wrong —
   * `pnpm backfill:affiliation-active-orgs` fills both fields and is a deploy
   * precondition.
   */
...
/**
 * The `org_status_ids` entry for one (org, status) pair — THE ONE PLACE this
 * key is spelled. The trigger writes it, the backfill repairs it and the
 * dashboard queries it; three hand-rolled template literals would agree right
 * up until somebody changed the separator.
 *
 * Neither half can contain a colon: an org id is a Firestore document id
 * generated by the console or a seeder, and a status id is one of the built-in
 * slugs or an org's own, written through `setAffiliationStatus`.
 */
...
export const DEFAULT_ORG_AFFILIATION_STATUSES: OrgAffiliationStatusDef[] = [
  {
    id: 'guest',
    label: 'Guest',
    description: 'No affiliation process started.',
    color: 'gray',
    order: 0,
    isBuiltIn: true,
